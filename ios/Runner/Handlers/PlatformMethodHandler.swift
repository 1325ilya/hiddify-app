...
public class DeviceIdentityMethodHandler: NSObject, FlutterPlugin {
    
    public static let name = "\(Bundle.main.serviceIdentifier)/device_identity"
    
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: Self.name, binaryMessenger: registrar.messenger())
        let instance = DeviceIdentityMethodHandler()
        registrar.addMethodCallDelegate(instance, channel: channel)
    }
